...
def extract_bill_text_from_soup(soup: BeautifulSoup) -> Optional[str]:
	...
	if heading:
		...
		for sib in heading.find_all_next():
			...
			if sib.name and sib.get("class") and any(
				c.lower().find("nav") != -1 or c.lower().find("vote") != -1 for c in sib.get("class")
			):
				break
			...
			if len(parts) > 20 and ("Votes" in txt or "Bill PDF" in txt or "Compare Versions" in txt):
				break
...
def main() -> None:
	parser = argparse.ArgumentParser(description="Extract bill legal text from a CA Legislature bill page")
	parser.add_argument("--url", required=False, default="https://leginfo.legislature.ca.gov/faces/billTextClient.xhtml?bill_id=202320240SB976", help="URL of the bill page")
