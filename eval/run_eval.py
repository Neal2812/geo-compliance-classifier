...
class EvaluationHarness:
    ...
    def __init__(self, 
                 ...
                 queries_file: str = "eval/queries.json"):
        """
    ...
    def run_evaluation(self, top_k: int = 5) -> EvaluationMetrics:
        ...
        for i, query_info in enumerate(self.queries):
            logger.info(f"Evaluating query {i+1}/{len(self.queries)}: {query_info['id']}")
        ...
        if metrics.successful_queries > 0:
            ...
            metrics.section_accuracy = section_correct_count / metrics.successful_queries
        
        if latencies:
            metrics.avg_latency_ms = statistics.mean(latencies)
            metrics.p50_latency_ms = statistics.median(latencies)
            metrics.p95_latency_ms = statistics.quantiles(latencies, n=20)[18]  # 95th percentile
    ...
    def _evaluate_query_results(self, 
                               ...
                               results: List[SearchResult]) -> Tuple[bool, bool, bool, bool]:
        """
    ...
    def _check_section_match(self, results: List[SearchResult], expected_sections: List[str]) -> bool:
    ...
    def _save_results(self, results_details: List[Dict[str, Any]], metrics: EvaluationMetrics):
    ...
    def benchmark_latency(self, 
        ...
        logger.info(f"Benchmarking latency with {len(sample_queries)} queries x {iterations} iterations")
        ...
        for query in sample_queries:
            ...
            for i in range(iterations):
                try:
                    ...
                    logger.error(f"Latency test failed for '{query}' iteration {i}: {e}")
...
def main():
    ...
    try:
        ...
        print(f"Success Rate: {metrics.successful_queries/max(1,metrics.total_queries):.2%}")
        ...
        print(f"P95 < 1000ms: {'✅ PASS' if metrics.p95_latency_ms < p95_target else '❌ FAIL'} ({metrics.p95_latency_ms:.0f}ms)")
        print(f"Hit@3 ≥ 90%: {'✅ PASS' if metrics.hit_at_3 >= hit3_target else '❌ FAIL'} ({metrics.hit_at_3:.1%})")
