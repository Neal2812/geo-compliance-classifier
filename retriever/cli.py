...
Examples:
  ...
  python -m retriever.cli health
    ...
    retrieve_parser = subparsers.add_parser(
        "retrieve", help="Search legal documents (default)"
    )
    retrieve_parser.add_argument("query", help="Search query")
    retrieve_parser.add_argument(
        "--laws", nargs="+", help="Filter by specific laws (e.g., EUDSA FL_HB3)"
    )
    retrieve_parser.add_argument(
    ...
    retrieve_parser.add_argument(
        "--no-score", action="store_true", help="Hide relevance scores"
    )
    retrieve_parser.add_argument(
        "--stats", action="store_true", help="Show query statistics"
    )
    ...
    law_parser.add_argument(
        "law", choices=["EUDSA", "CA_SB976", "FL_HB3", "US_2258A"], help="Law to search"
    )
    law_parser.add_argument(
        "--k", type=int, default=3, help="Number of results (default: 3)"
    )
    law_parser.add_argument(
        "--no-score", action="store_true", help="Hide relevance scores"
    )
    ...
    jurisdiction_parser = subparsers.add_parser(
        "search-jurisdiction", help="Search within jurisdiction"
    )
    jurisdiction_parser.add_argument("query", help="Search query")
    jurisdiction_parser.add_argument(
        "jurisdiction",
        choices=["EU", "US-CA", "US-FL", "US"],
