...
from .schema import CodenameHit, FeatureRecord
...
class ProcessingReporter:
    ...
    def _calculate_stats(
        self, records: List[FeatureRecord], docs_found: int, terminology_size: int
    ) -> None:
        ...
        self.stats.update(
            {
                ...
                "parse_success_rate": (
                    docs_parsed / docs_found if docs_found > 0 else 0.0
                ),
                "features_processed": len(records),
                "total_warnings": len(all_warnings),
                "expansion_coverage": (
                    len(unique_terms) / terminology_size
                    if terminology_size > 0
                    else 0.0
                ),
    ...
    def _build_report_content(self, records: List[FeatureRecord]) -> str:
        ...
        if records:
            ...
            for field_name, (filled, total) in sorted(field_stats.items()):
                ...
                lines.append(
                    f"- **{field_name}:** {filled}/{total} ({percentage:.1f}%)"
                )
            lines.append("")
    ...
    def _calculate_field_stats(self, records: List[FeatureRecord]) -> Dict[str, tuple]:
        ...
        for field_name in field_names:
            filled_count = sum(
                1 for record in records if getattr(record, field_name, None)
            )
    ...
    def _categorize_warning(self, warning: str) -> str:
        ...
        if any(keyword in warning_lower for keyword in ["pdf", "pypdf", "pdfminer"]):
            ...
            keyword in warning_lower for keyword in ["encoding", "decode", "unicode"]
        ):
