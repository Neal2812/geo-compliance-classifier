...
def detect_encoding(file_path: Path) -> str:
    ...
    try:
        ...
        logger.debug(
            f"Detected encoding '{encoding}' (confidence: {confidence:.2f}) for {file_path}"
        )

        # Fallback to utf-8 if confidence is too low
        if confidence < 0.7:
            logger.warning(
                f"Low confidence encoding detection for {file_path}, using utf-8"
            )
            return "utf-8"
...
def read_text_file(file_path: Path, encoding: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.warning(
            f"Failed to decode {file_path} with {encoding}, trying utf-8 with errors='ignore'"
        )
...
def load_terminology_csv(file_path: Path) -> Dict[str, str]:
    ...
    try:
        with open(file_path, "r", encoding=encoding) as f:
            ...
            if (
                "term" not in reader.fieldnames
                or "explanation" not in reader.fieldnames
            ):
                raise ValueError(
                    f"CSV must have 'term' and 'explanation' columns. Found: {reader.fieldnames}"
                )
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                if not explanation:
                    logger.warning(
                        f"Empty explanation for term '{term}' in row {row_num}"
                    )
...
def load_features_csv(file_path: Path) -> List[Tuple[str, str]]:
    ...
    try:
        with open(file_path, "r", encoding=encoding) as f:
            ...
            # Validate required columns
            if (
                "feature_name" not in reader.fieldnames
                or "feature_description" not in reader.fieldnames
            ):
                raise ValueError(
                    f"CSV must have 'feature_name' and 'feature_description' columns. Found: {reader.fieldnames}"
                )
    ...
    with open(output_path, "w", encoding="utf-8", newline="") as f:
        ...
        for record in records:
            ...
            if isinstance(row_dict.get("codename_hits_json"), list):
                row_dict["codename_hits_json"] = json.dumps(
                    row_dict["codename_hits_json"]
                )
...
def find_documents(
    docs_dir: Path, extensions: Optional[List[str]] = None
) -> List[Path]:
