...
def main():
    ...
    if not openai_api_key:
        print("⚠️  Warning: OPENAI_API_KEY not set. LLM+RAG model will use fallback mode.")
    ...
    for case in test_cases:
        ...
        for model_name, prediction in result.predictions.items():
            status_emoji = "✅" if prediction.confidence >= 0.85 else "⚠️" if prediction.confidence >= 0.70 else "❌"
            print(f"    {status_emoji} {model_name}: {prediction.decision} ({prediction.confidence:.2f})")
