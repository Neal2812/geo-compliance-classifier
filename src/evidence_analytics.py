...
class EvidenceAnalytics:
    ...
    def get_compliance_trends(self, 
                             df: pd.DataFrame,
                             time_group: str = 'date') -> Dict[str, Any]:
        """
        ...
        grouped['compliance_rate'] = (grouped['compliant_decisions'] / grouped['total_decisions']).round(4)
    ...
    def get_agent_performance(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Flatten column names
        agent_stats.columns = [
            'total_decisions', 'compliant_decisions', 
        ...
        # Performance ranking
        agent_stats['performance_rank'] = agent_stats['compliance_rate'].rank(ascending=False)
        ...
        return {
            ...
            'performance_gap': float(
                agent_stats['compliance_rate'].max() - agent_stats['compliance_rate'].min()
            )
        }
    ...
    def get_regulatory_impact(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        high_impact = {
            reg: data for reg, data in regulation_compliance.items()
        ...
        return {
            ...
            'compliance_variance': float(
                pd.Series([data['compliance_rate'] for data in regulation_compliance.values()]).std()
            ) if regulation_compliance else 0.0
        }
    ...
    def get_performance_insights(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        timing_columns = [col for col in df.columns if 'timings_ms' in col or 'ms' in col]
        timing_insights = {}
                ...
                if not timing_data.empty:
                    ...
                    for item in timing_data:
                        if isinstance(item, dict):
                            numeric_times.extend([v for v in item.values() if isinstance(v, (int, float))])
        ...
        volume_analysis = {
            ...
            'decisions_per_day': len(df) / max(1, (df['timestamp'].max() - df['timestamp'].min()).days),
            'peak_hour': int(df['hour'].mode().iloc[0]) if not df['hour'].empty else None,
            'busiest_day': df['day_of_week'].mode().iloc[0] if not df['day_of_week'].empty else None
        }
        ...
        confidence_analysis = {
            ...
            'confidence_distribution': {
                ...
                'medium': float(len(df[(df['confidence'] >= 0.5) & (df['confidence'] < 0.8)])),
        ...
        for col, insights in timing_insights.items():
            if insights['p95'] > insights['bottleneck_threshold']:
                bottlenecks.append({
                    ...
                    'severity': 'high' if insights['p95'] > insights['bottleneck_threshold'] * 2 else 'medium'
                })
        ...
        if 'timings_ms' in df.columns:
            ...
            if not timing_data.empty:
                ...
                slow_responses = timing_data.apply(
                    lambda x: any(v > 1000 for v in x.values() if isinstance(v, (int, float)))
                )
