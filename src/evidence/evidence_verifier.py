...
class EvidenceVerificationAgent:
    ...
    def _initialize_compliance_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            "strong_compliance": [
                ...
                "compliance", "comply", "conform", "adhere", "enforce"
            ],
            "prohibited": [
                "prohibited", "forbidden", "illegal", "unlawful", "banned",
                "restricted", "not permitted", "not allowed"
            ],
            "regulatory_framework": [
                "regulation", "statute", "law", "act", "code", "standard",
                "guideline", "policy", "requirement"
            ],
            "enforcement": [
                "penalty", "fine", "sanction", "enforcement", "violation",
                "breach", "non-compliance", "audit", "review"
            ]
        }
    
    def verify_case(self, case_id: str, reasoning_text: str, 
                    evidence_spans: List[Dict[str, Any]], 
                    regulation_references: List[str]) -> VerificationResult:
        """
        ...
        final_decision, auto_approved, flags, notes, overall_score = self._make_verification_decision(
    ...
    def _validate_reasoning_alignment(self, reasoning_text: str, 
                                    evidence_spans: List[EvidenceSpan]) -> ReasoningValidation:
    ...
    def _validate_regulation_mappings(self, regulation_references: List[str], 
                                    evidence_spans: List[EvidenceSpan]) -> List[RegulationMapping]:
    ...
    def _normalize_regulation_name(self, regulation_ref: str) -> str:
        ...
        prefixes_to_remove = [
            "the ", "act", "regulation", "statute", "law", "code"
        ]
        ...
        for key in self.regulation_database.keys():
            if regulation_ref.lower() in key.lower() or key.lower() in regulation_ref.lower():
                return key
        
        return normalized
    
    def _check_regulation_reference(self, regulation_content: str, 
                                  evidence_spans: List[EvidenceSpan]) -> bool:
    ...
    def _assess_evidence_quality(self, evidence_spans: List[EvidenceSpan]) -> List[EvidenceQuality]:
        """Assess the quality of evidence spans"""
        quality_assessments = []
        
        for span in evidence_spans:
            if not span.text.strip():
                quality_assessments.append(EvidenceQuality(
                    span=span,
                    quality_score=0.0,
                    quality_level="Weak",
            ...
            quality_assessments.append(EvidenceQuality(
                ...
                quality_notes=self._generate_quality_notes(span.text, compliance_terms, specific_language)
            ))
    ...
    def _calculate_quality_score(self, compliance_terms: List[str], 
                               specific_language: bool, 
                               regulation_linked: bool, 
    ...
    def _generate_quality_notes(self, text: str, compliance_terms: List[str], 
                               specific_language: bool) -> str:
        """Generate notes about evidence quality"""
        notes = []
        
        if compliance_terms:
            notes.append(f"Contains {len(compliance_terms)} compliance terms: {', '.join(compliance_terms[:3])}")
    ...
    def _make_verification_decision(self, reasoning_validation: ReasoningValidation,
                                  regulation_mappings: List[RegulationMapping],
                                  evidence_quality: List[EvidenceQuality]) -> Tuple[str, bool, List[str], str, float]:
        ...
        weak_evidence = [eq for eq in evidence_quality if eq.quality_level in ["Weak", "Generic"]]
        if weak_evidence:
        ...
        alignment_score = reasoning_validation.alignment_score
        regulation_score = sum(1 for m in regulation_mappings if m.is_valid) / max(1, len(regulation_mappings))
        quality_score = sum(eq.quality_score for eq in evidence_quality) / max(1, len(evidence_quality))
    ...
    def get_verification_summary(self) -> str:
        ...
        for result in self.verification_history:
            row = {
                ...
                "Regulation Mapping Valid": "Yes" if result.regulation_mapping_valid else "No",
    ...
    def export_verification_results(self, filename: str = None) -> str:
        ...
        if filename is None:
            from datetime import datetime
            filename = f"evidence_verification_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        for result in self.verification_history:
            ...
            for eq in result.evidence_quality:
                markdown_content += f"- {eq.span.source}: {eq.quality_level} ({eq.quality_score:.2f})\n"
