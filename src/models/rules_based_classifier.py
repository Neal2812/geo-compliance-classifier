...
class RulesBasedClassifier:
    ...
    def _initialize_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            "compliant": [
                "compliant", "compliance", "legal", "approved", "permitted", 
                "authorized", "valid", "proper", "correct", "adequate"
            ],
            "non_compliant": [
                "non-compliant", "illegal", "prohibited", "violation", "breach",
                "unauthorized", "invalid", "improper", "incorrect", "inadequate"
            ],
            "uncertain": [
                "unclear", "uncertain", "ambiguous", "unclear", "depends",
                "case-by-case", "context-dependent", "review required"
            ]
        }
    ...
    def _combine_approaches(self, rule_matches: List[ComplianceRule], 
                           keyword_scores: Dict[str, float]) -> Tuple[str, float]:
        """Combine rule-based and keyword-based approaches for final decision"""
        
        if not rule_matches and not any(score > 0.1 for score in keyword_scores.values()):
            return "Unclear", 0.50
        ...
        if rule_matches:
            ...
            if len(rule_matches) > 1:
                agreeing_rules = [r for r in rule_matches if r.decision == best_rule.decision]
