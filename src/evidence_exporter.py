...
class EvidenceExporter:
    ...
    def read_evidence_records(self, 
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    for line_num, line in enumerate(f, 1):
                        ...
                        try:
                            ...
                            # Apply filters
                            if not self._passes_filters(record, start_date, end_date, agent_filter):
                                continue
    ...
    def _passes_filters(self, record: Dict[str, Any], 
                       start_date: Optional[datetime],
                       end_date: Optional[datetime],
                       agent_filter: Optional[List[str]]) -> bool:
        """Check if record passes all filters."""
        # Date filter
        if start_date or end_date:
            try:
                record_timestamp = datetime.fromisoformat(record.get('timestamp_iso', ''))
    ...
    def transform_to_test_dataset_schema(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if isinstance(retrieval_meta, dict):
            if 'retrieved_ids' in retrieval_meta:
                retrieved_ids = retrieval_meta['retrieved_ids'] if isinstance(retrieval_meta['retrieved_ids'], list) else []
            if 'scores' in retrieval_meta:
                scores = retrieval_meta['scores'] if isinstance(retrieval_meta['scores'], list) else []
        ...
        llm_model_name = model_meta.get('llm_model_name', '') if isinstance(model_meta, dict) else ''
        prompt_template_id = model_meta.get('prompt_template_id', '') if isinstance(model_meta, dict) else ''
        
        # Extract embedding information
        embedder_name = retrieval_meta.get('embedder_name', '') if isinstance(retrieval_meta, dict) else ''
        embed_dim = retrieval_meta.get('embed_dim', '') if isinstance(retrieval_meta, dict) else ''
        vectorstore_type = retrieval_meta.get('vectorstore_type', '') if isinstance(retrieval_meta, dict) else ''
        metric = retrieval_meta.get('metric', '') if isinstance(retrieval_meta, dict) else ''
        normalize = retrieval_meta.get('normalize', '') if isinstance(retrieval_meta, dict) else ''
        top_k = retrieval_meta.get('top_k', '') if isinstance(retrieval_meta, dict) else ''
    ...
    def export_to_csv(self, 
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for record in self.read_evidence_records(start_date, end_date, agent_filter, limit):
    ...
    def export_test_dataset_csv(self,
                               output_path: str,
        ...
        test_schema = [
            'request_id', 'timestamp_iso', 'agent_name', 'feature_id', 'feature_title', 'dataset_tag',
            'decision_flag', 'reasoning_text', 'related_regulations', 'confidence',
            'llm_model_name', 'prompt_template_id',
            'embedder_name', 'embed_dim', 'vectorstore_type', 'metric', 'normalize', 'top_k',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Export evidence logs to challenge CSV format')
    ...
    parser.add_argument('--format', choices=['csv', 'dataframe'], default='csv',
                       help='Output format')
    parser.add_argument('--test-dataset', action='store_true', 
                       help='Export in test dataset format')
    parser.add_argument('--dataset-tag', help='Filter by dataset tag for test dataset export')
    ...
    if args.format == 'csv':
        if args.test_dataset:
            count = exporter.export_test_dataset_csv(
                args.output, args.dataset_tag, start_date, end_date, args.agents, args.limit
            )
        ...
        df = exporter.export_to_dataframe(
            start_date, end_date, args.agents, args.limit
        )
