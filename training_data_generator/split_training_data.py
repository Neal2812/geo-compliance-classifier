...
class DatasetSplitter:
    ...
    def load_corrections(self) -> pd.DataFrame:
        ...
        for correction in corrections:
            record = {
                ...
                'state': correction['feature_characteristics']['geographic'].get('state', 'Unknown'),
                'country': correction['feature_characteristics']['geographic'].get('country', 'Unknown'),
                'region': correction['feature_characteristics']['geographic'].get('region', 'Unknown'),
                
                # Extract demographic features
                'age_min': correction['feature_characteristics']['demographic'].get('age_min', 0),
                'age_max': correction['feature_characteristics']['demographic'].get('age_max', 100),
                
                # Extract regulation info
                'regulation_type': correction['feature_characteristics'].get('regulation_type', 'Unknown'),
                'feature_type': correction['feature_characteristics'].get('feature_type', 'Unknown'),
            }
    ...
    def analyze_data_distribution(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'original_predictions': self.data['original_prediction'].value_counts().to_dict(),
    ...
    def create_splits(self, train_ratio: float = 0.7, test_ratio: float = 0.2, val_ratio: float = 0.1, 
                     stratify_column: str = 'corrected_label', random_state: int = 42) -> Dict[str, pd.DataFrame]:
        ...
        logger.info(f"Creating splits: {train_ratio:.1%} train, {test_ratio:.1%} test, {val_ratio:.1%} validation")
        ...
        if min_class_size < 2 or len(self.data) < 10:
            logger.warning("Dataset too small for stratified splitting. Using random split.")
...
def main():
    ...
    logger.info(f"  Geographic coverage: {len(analysis['geographic_distribution']['countries'])} countries")
