...
class ComplianceRuleEngine:
    ...
    def _load_rules(self) -> Dict:
        ...
        rules['US-FL'] = {
            ...
            'triggers': {
                'banned_under_14': [
                    'account_creation', 'social_features', 'content_sharing'
                ],
                'consent_required_14_15': [
                    'data_collection', 'personalization', 'social_features'
                ],
        ...
        rules['US-CA'] = {
            ...
            'triggers': {
                ...
                'default_safety_settings': [
                    'privacy_controls', 'content_filtering', 'time_limits'
                ],
        ...
        rules['EU'] = {
            ...
            'triggers': {
                ...
                'moderation_notices': [
                    'content_removal', 'account_restrictions', 'appeal_processes'
                ],
                'accessible_terms': [
                    'terms_of_service', 'privacy_policies', 'user_agreements'
                ]
            },
            'mitigations': {
        ...
        rules['US-2258A'] = {
            ...
            'triggers': {
                ...
                'preservation_required': [
                    'evidence_preservation', 'metadata_retention'
                ],
                'no_proactive_scanning': [
                    'automated_content_scanning', 'preemptive_detection'
                ]
            },
            'mitigations': {
    ...
    def evaluate_compliance(self, feature: Dict, geo_country: str, geo_state: str = None) -> Tuple[str, str, List[str]]:
        ...
        for trigger_category, trigger_list in rules['triggers'].items():
            feature_triggers = feature.get('addictive_features', []) + feature.get('data_practices', []) + feature.get('risk_tags', [])
        ...
        if not violations:
            return "Compliant", f"Feature meets all {rules['name']} requirements", implicated_regs
        ...
        # Non-compliant
        violation_desc = ', '.join([f"{cat} ({trigger})" for cat, trigger in violations[:3]])
...
def main_checklist():
    ...
    print(f"🔍 Compliance Engine: {len(ComplianceRuleEngine().rules)} jurisdictions supported")
