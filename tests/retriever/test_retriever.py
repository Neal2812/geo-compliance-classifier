...
from retriever.models import Jurisdiction, LegalDocument, TextChunk
...
def test_search_models():
    ...
    request = RetrievalRequest(
        query="test query", laws=["EUDSA"], top_k=5, max_chars=1000
    )
...
def test_hybrid_retriever():
    ...
    results = retriever.retrieve(
        query="risk assessment platforms", dense_scores=dense_scores, top_k=2
    )
...
def test_query_tokenization():
    ...
    legal_text = (
        "Section §501.1736(2)(a) requires parental consent for Art. 28 compliance"
    )
