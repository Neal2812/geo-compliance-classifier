...
def test_index_building():
    ...
    try:
        ...
        if (index_dir / "vector.faiss").exists() and (
...
def test_retrieval_service():
    ...
    try:
        ...
        if results:
            print(
                f"   🏆 Top result: {results[0].law_id} (score: {results[0].score:.3f})"
            )
    ...
    try:
        ...
        for i, test_query in enumerate(queries, 1):
            ...
            retrieval_results = service.retrieve(
                query=test_query["query"], top_k=5, max_chars=800
            )
            latency = (time.time() - start_time) * 1000
            ...
            query_result = {
                ...
                "top_result": (
                    {
                        "law_id": (
                            retrieval_results[0].law_id if retrieval_results else None
                        ),
                        "score": retrieval_results[0].score if retrieval_results else 0,
                        "section": (
                            retrieval_results[0].section_label
            ...
            print(f"   ⏱️  Latency: {latency:.1f}ms")
            print(
                f"   🎯 Hit@1: {'✅' if hit_1 else '❌'} | Hit@3: {'✅' if hit_3 else '❌'}"
            )
            if retrieval_results:
                print(
                    f"   🏆 Top: {retrieval_results[0].law_id} ({retrieval_results[0].score:.3f})"
                )
        ...
        print(
            f"   • Hit@1: {results['hit_at_1']}/{total_queries} ({hit_at_1_rate:.1%})"
        )
        print(
            f"   • Hit@3: {results['hit_at_3']}/{total_queries} ({hit_at_3_rate:.1%})"
        )
        print(
            f"   • Hit@5: {results['hit_at_5']}/{total_queries} ({hit_at_5_rate:.1%})"
        )
        ...
        print(f"\n🎯 Target Assessment:")
        print(
            f"   • P95 < 1000ms: {'✅' if performance_target_met else '❌'} ({p95_latency:.1f}ms)"
        )
        print(
            f"   • Hit@3 ≥ 80%: {'✅' if accuracy_target_met else '❌'} ({hit_at_3_rate:.1%})"
        )
...
def main():
    ...
    if eval_results:
        ...
        p95_latency = (
            statistics.quantiles(eval_results["latencies"], n=20)[18]
            if len(eval_results["latencies"]) >= 20
            else max(eval_results["latencies"])
        )
