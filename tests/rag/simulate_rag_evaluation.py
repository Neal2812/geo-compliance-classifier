...
def create_mock_evaluation_results():
    ...
    for i, test_query in enumerate(queries, 1):
        ...
        retrieved_laws.extend(
            random.sample(remaining_laws, min(4, len(remaining_laws)))
        )
        ...
        if retrieved_laws:
            print(
                f"   🏆 Top: {retrieved_laws[0]} ({query_result['top_result']['score']:.3f})"
            )
        print()
...
def analyze_legal_texts():
    ...
    print(
        f"📈 Total content: {total_content:,} characters across {len(legal_files)} laws"
    )
...
def main():
    ...
    if not content_available:
        print(
            "❌ No legal content found. Please ensure legal_texts/ directory contains the required files."
        )
        return
    ...
    print(
        f"   • Hit@1: {eval_results['hit_at_1']}/{total_queries} ({hit_at_1_rate:.1%})"
    )
    print(
        f"   • Hit@3: {eval_results['hit_at_3']}/{total_queries} ({hit_at_3_rate:.1%})"
    )
    print(
        f"   • Hit@5: {eval_results['hit_at_5']}/{total_queries} ({hit_at_5_rate:.1%})"
    )
    ...
    print(
        f"   • Hit@3 ≥ 90%: {'✅' if accuracy_target_met else '❌'} ({hit_at_3_rate:.1%})"
    )
    ...
    overall_status = (
        ...
        if accuracy_target_met and performance_target_met
