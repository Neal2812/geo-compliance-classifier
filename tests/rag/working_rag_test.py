...
class SimpleTextRetriever:
    ...
    def retrieve(
        self, query: str, laws: List[str] = None, top_k: int = 5, max_chars: int = 800
    ) -> List[SimpleResult]:
...
def run_working_evaluation():
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        if search_results:
            ...
            print(
                f"   🏆 Top result: {search_results[0].law_id} (score: {search_results[0].score:.3f})"
            )
    ...
    if results["latencies"]:
        ...
        print(
            f"📊 Hit@1: {results['hit_at_1']}/{results['total_queries']} ({hit_1_rate:.1%})"
        )
        print(
            f"📊 Hit@3: {results['hit_at_3']}/{results['total_queries']} ({hit_3_rate:.1%})"
        )
